...
public class WebIATWS extends WebSocketListener {
    ...
    private static final String file = "src/main/resources/audio/1.pcm"; // 中文
    ...
    public WebIATWS(CountDownLatch latch) {
        this.latch = latch;
    }
    @Override
    ...
    public String connect() throws Exception {
        ...
        client.newWebSocket(request, new WebIATWS(latch));
        // 等待websockt处理完成
        try {
            latch.await();
        } catch (InterruptedException e) {
            e.printStackTrace();
            return InterruptedException.class.toString();
        }
        if (audioResult != null) {
            System.out.println("完成:" + audioResult);
            return audioResult;
        } else {
            // 如果 audioResult 为 null，抛出异常或处理错误情况
            throw new Exception("Audio result is not available.");
        }
    }
