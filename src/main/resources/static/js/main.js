...
export function createMenu(sessionTitle) {
  ...
  menuIcon.onclick = function (event) {
    ...
    deleteItem.onclick = async function () {
      menu.style.display = 'none';
      const sessionId = sessionTitle.getAttribute('data-session-id');
      if (!isOnlyWeb) {
        try {
          const turnstile = await _turnstileCb();
          const result = await fetchGetResults(`${baseUrl}/api/deleteConverById?conversationId=${sessionId}`, getCookie(AUTH_TOKEN), turnstile)
          if (result.code === 200) {
            showAlert('删除成功', true);
...
function renameSession(sessionTitle) {
  ...
  // 监听blur事件来应用新的名称
  titleTextSpa.onblur = async function () {
    ...
    if (newText === '') {
      ...
      if (!isOnlyWeb) {
        try {
          const sessionId = sessionTitle.getAttribute('data-session-id');
          const turnstile = await _turnstileCb();
          const result = await fetchGetResults(`${baseUrl}/api/updateTitle?conversationId=${sessionId}&title=${newText}`, getCookie(AUTH_TOKEN), turnstile)
          if (result.code === 200) {
            showAlert('更新成功', true);
          } else {
            showAlert('更新失败', false);
          }
        } catch (error) {
          showAlert('请求异常', false);
        }
...
const CONFIG = {
  userPath: "./svg/avatar-user.svg",
  systemPath: "./svg/logo.svg",
  systemName: "更勤奋更聪明的GPT4",
  timeout: 120000
};
...
// 3. 优化消息处理函数
async function handleMessageStream(reader, replyElement, chatBoxOne) {
  ...
  try {
    while (true) {
      ...
      if (done) {
        return finishMessageProcessing(replyElement, chatBoxOne);
      }

      accumulatedText += decoder.decode(value);

      while ((pos = accumulatedText.indexOf("\n\n")) !== -1) {
        message = accumulatedText.slice(0, pos).trim();
        accumulatedText = accumulatedText.slice(pos + 2);

        await processMessageChunk(message, replyElement, chatBoxOne);
      }
    }
  } catch (error) {
    handleStreamError(error, chatBoxOne);
  }
}
...
function setupEventListeners() {
  ...
  DOM.messageInput.addEventListener("keydown", handleMessageInputKeydown);
  window.addEventListener("click", handleGlobalClick);

  // 使用事件委托处理动态创建的元素
  document.body.addEventListener("click", (e) => {
    const target = e.target;
    if (target.matches(".copy-button")) handleCopyClick(e);
    if (target.matches(".translate-button")) handleTranslateClick(e);
  });
}
...
const handleError = (error, type = 'general') => {
  ...
  showAlert(errorMessages[type] || error.message, false);
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateSessionTitle(sessions) {
    sessions.forEach((session, index) => {
      ...
      sessionTitle.addEventListener('click', function () {
        if (getIsMoreChat()) {
          setIsMoreChat(!getIsMoreChat());
          var headTitleDiv = document.querySelector('.head-title');
          var spanElement = headTitleDiv.querySelector('span');
          spanElement.textContent = 'AI竞技场';
          const fightContainer = document.getElementById('chat-fight-container');
          const chatContainer = document.getElementById('chat-areas-container');
          const fightSend = document.getElementById('fight-send');
          const chatInput = document.getElementById('input-area');
          const chatContent = document.getElementById('chat-content');
          chatContent.style.height = 62 + 'vh';
          fightContainer.style.display = 'none';
          fightSend.style.display = 'none';
  ...
  function displayUserMessage(message, chatBox) {
    ...
    const htmlMessage = message.replace(/\\s/g, ' ').replace(/\\n/g, '\\n').replace(/\\"/g, '"');
    const textElement = document.createElement("div");
    textElement.classList.add("sent-message", "received");
    textElement.textContent = htmlMessage;
  ...
  document.addEventListener('click', async function (e) {
    ...
    if (e.target.tagName === "IMG" && !e.target.alt.includes("icon") && !e.target.alt.includes("GPT")) {
      ...
      if (isValidUrl(e.target.src)) {
        const imgUrl = await loadImage(e.target.src);
        modalImg.src = imgUrl;
      } else {
  ...
  function modelFill(models, element) {
    models.forEach(model => {
      ...
      element.appendChild(modelDiv);
    });
  }

  modelFill(models, dropdownContent);
  // 填充模型到下拉菜单
  // models.forEach(model => {
  //   const modelDiv = document.createElement("div");
  //   modelDiv.textContent = model;
  ...
  function createAvatarAndUserName({ isUser, name = "ChatGPT", imgPath }) {
    ...
    if (isUser) {
      ...
      text.setAttribute('font-size', '600');
  ...
  async function postMessageAndHandleResponse(url, formData, chatBoxset, timeout = 120000) {
    ...
    try {
      let resp;
      if (isOnlyWeb || parsedMessageRe.gizmo !== '') {
        if (loadFromLocalStorage(API_KEY) === null || loadFromLocalStorage(API_URL) === null) {
          saveToLocalStorage(API_KEY, audioKey);
          saveToLocalStorage(API_URL, audioUrl);
          // sendButton.style.backgroundImage = '';
          // sendButton.innerText = '发送';
          // isSend = false;
          // spinnerElement.remove();
        ...
        const data = JSON.stringify({
          ...
          "stream": true,
          "temperature": parseFloat(globalModeSettings.tempertaure),
          "top_p": parseFloat(globalModeSettings.topP),
          "max_tokens": parseInt(globalModeSettings.maxTokens)
        });
        resp = await fetch(`${loadFromLocalStorage(API_URL)}/v1/chat/completions`, {
          method: 'POST',
          headers: {
        ...
        while (true) {
          ...
          while ((pos = accumulatedText.indexOf("\n\n")) !== -1) {
            ...
            if (message.startsWith('data:')) {
              ...
              chatBoxOne.setAttribute("message-id", messageId);
              if (messageJson.conversationId && messageJson.isFirstFlag) {
              ...
              if (messageJson.isFirstFlag && !getIsMoreChat()) {
                ...
                try {
                  ...
                  if (existingTitleTextSpan) {
                    ...
                    var spanElement = headTitleDiv.querySelector('span');
                    spanElement.textContent = '';
                    typeWriter(spanElement, data.data, 100);
                  }
                } catch (error) {
                  isFirstFlag = true;
      ...
      const receiveMessageContainer = document.createElement("div");
      receiveMessageContainer.classList.add("receive-message-container");
      const errorMessageText = JSON.stringify(`{'errorMessage': '${error}'}`);
      errorMessage(chatBoxOne, errorMessageText, replyElement, receiveMessageContainer);
    }
  }
  ...
  let audio = null; // 全局变量来保存音频对象
  let isPaused = false; // 标记当前是否暂停
  /**
   * 文本转语音
   * @param {*} message
   * @param {*} selectVoice
  ...
  async function audioMessage(message, selectVoice, vocieButton) {
    try {
      ...
      let raw, url, auth_token, turnstile;
      ...
      if (isOnlyWeb) {
        ...
        turnstile = await _turnstileCb();
      }
      const audioUrls = await postAudiceResults(url, auth_token, isOnlyWeb ? raw : formData, turnstile);
      // const audioUrl = URL.createObjectURL(audiceBlob);
      audio = new Audio(audioUrls);
      ...
      <span class="flex h-[30px] w-[30px] items-center justify-center voice">
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="icon-md-heavy">
          ...
          <g class="wave1">
            <path fill-rule="evenodd" clip-rule="evenodd" d="M20.3166 6.35665C20.8019 6.09313 21.409 6.27296 21.6725 6.75833C22.5191 8.3176 22.9996 10.1042 22.9996 12.0001C22.9996 13.8507 22.5418 15.5974 21.7323 17.1302C21.4744 17.6185 20.8695 17.8054 20.3811 17.5475C19.8927 17.2896 19.7059 16.6846 19.9638 16.1962C20.6249 14.9444 20.9996 13.5175 20.9996 12.0001C20.9996 10.4458 20.6064 8.98627 19.9149 7.71262C19.6514 7.22726 19.8312 6.62017 20.3166 6.35665Z" fill="currentColor"></path>
          </g>
          <g class="wave2">
            <path fill-rule="evenodd" clip-rule="evenodd" d="M15.7994 7.90049C16.241 7.5688 16.8679 7.65789 17.1995 8.09947C18.0156 9.18593 18.4996 10.5379 18.4996 12.0001C18.4996 13.3127 18.1094 14.5372 17.4385 15.5604C17.1357 16.0222 16.5158 16.1511 16.0539 15.8483C15.5921 15.5455 15.4632 14.9255 15.766 14.4637C16.2298 13.7564 16.4996 12.9113 16.4996 12.0001C16.4996 10.9859 16.1653 10.0526 15.6004 9.30063C15.2687 8.85905 15.3578 8.23218 15.7994 7.90049Z" fill="currentColor"></path>
          </g>
        </svg>
      </span>
    `;
...
`;
        ...
        const hailuoVoices = ['Leishen2_hailuo', 'Zhongli_hailuo', 'YaeMiko_hailuo', 'Hutao_hailuo', 'Haimian_hailuo', 'Xionger_hailuo']
        voices.forEach(voice => {
          ...
          voiceButton.onclick = function () {
            ...
            audioMessage(message, voice, vocieButton);
...
`;
  ...
  // 发送消息逻辑
  async function sendMessage(lastContent, chatBox, model) {
    ...
    if (message || message === "") {
      ...
      messageRe.conversationId = getActiveSessionId();
      messageRe.messageId = messageId;
      messageRe.model = model || selectmodel;
      ...
      //   messageRe.content = messageRe.content;
      // } else {
  ...
  function applyTheme(theme) {
    const search = document.querySelector('.gsc-control-cse');
    if (search) {
      if (theme === 'dark-theme') {
        search.style.backgroundColor = '#303030';
        search.style.borderColor = '#303030';
        themeBtn.setAttribute('src', './svg/light-theme.svg');
      } else {
        search.style.backgroundColor = '#fff';
        search.style.borderColor = '#fff';
        themeBtn.setAttribute('src', './svg/dark-theme.svg');
      }
    }
  }
    ...
    mutations.forEach(function (mutation) {
      if (mutation.addedNodes) {
        mutation.addedNodes.forEach(function (node) {
          if (node.matches && node.matches('.gsc-control-cse')) {
  ...
  async function sendAudioToServer(audioBlob) {
    const formData = new FormData();
    if (isOnlyWeb) {
      formData.append("file", audioBlob, "a.wav");
  ...
  clearBtn.addEventListener('click', async function () {
    try {
      const turnstile = await _turnstileCb();
      const result = await fetchGetResults(`${baseUrl}/api/clearConver/${getActiveSessionId()}`, getCookie(AUTH_TOKEN), turnstile);
      if (result.code === 200) {
        showAlert(result.msg, true);
      } else {
        showAlert(result.msg, false);
      }

    } catch (error) {
      showAlert('请求异常: ' + error, false)
    }
  });
   ...
   * 初始化对话页面
   */
  document.getElementById('sidebar-menu-fire').addEventListener('click', function () {
    const fightContainer = document.getElementById('chat-fight-container');
    const chatContainer = document.getElementById('chat-areas-container');
    const fightSend = document.getElementById('fight-send');
    const chatInput = document.getElementById('input-area');
    const chatContent = document.getElementById('chat-content');
    let isMoreChat = getIsMoreChat();
    setIsMoreChat(!isMoreChat);
    if (getIsMoreChat()) {
      var headTitleDiv = document.querySelector('.head-title');
      var spanElement = headTitleDiv.querySelector('span');
      spanElement.textContent = 'AI竞技场';
  ...
  button.addEventListener('click', function (e) {
    ...
    textarea.value = '';
    chatContent.style.height = 8 + 'vh';
    textarea.dispatchEvent(new Event('input'));
    const boxs = document.querySelectorAll('.chat-fight-box');
    executeSendMessageSimultaneously(value, boxs);
  });
  const inputbtn = document.getElementById('fight-send');
  ...
  function executeSendMessageSimultaneously(value, boxes) {
    boxes.forEach(box => {
      const mId = box.getAttribute('mx-id');
      const btn = document.getElementById(mId);
      const model = btn.textContent;
      console.log(btn.textContent);
      requestAnimationFrame(() => {
        sendMessage(value, box, model);
      });
    });
  }
});
