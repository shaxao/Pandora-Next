...
function openModal(type) {
  ...
  if (type === 'register') {
    ...
    <label for="password" class="block mb-2 text-token-text-secondary">密码
      (8-12数字、大小写字母):</label>
    <input type="password" name="password" id="password" required pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z])[A-Za-z0-9.\-]{8,12}"
      class="w-full  mb-4 rounded-xl border bg-token-main-surface-primary px-2 py-2.5 text-lg text-token-text-tertiary"
      placeholder="输入密码">
...
document.getElementById('logOut').addEventListener('click', async function () {
  try {
    ...
    const data = await postResults(`${baseUrl}/api/user/logout`, getCookie(AUTH_TOKEN), null, turnstile);
    if (data.code == 200) {
...
registerBtn.addEventListener('click', function (event) {
  ...
  document.getElementById('verfityBtn').addEventListener('click', function () {
    ...
    const url = `${baseUrl}/api/user/email/code?email=${email}`;
    fetch(url, {
      method: 'GET'
    })
      .then(response => response.json())
